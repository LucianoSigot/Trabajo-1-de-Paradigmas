...
function darNombre() { // Función que pide el nombre al usuario
    const nombre = prompt("Ingrese su nombre:");
    if (!nombre || nombre.trim() === "") {//Controlo que no ingrese vacio
...
function mostrarMenuPrincipal() {//Menú principal
    let op;
    ...
    do {
        ...
        if (!op || op.trim() === "") {//Controlo que no ingrese vacio
        ...
        switch (op) {
            case "1":
                ...
                break;
            case "2":
                if (!aTareas || aTareas.length === 0) {//Controlo que tenga tareas cargadas anteriormente
                    console.log("No hay tareas cargadas. Por favor, agregue una tarea antes de buscar.");
                } else {
                    buscarTarea(aTareas);//Si hay tareas disponibles realizo la busqueda
                }
                break;
            case "3":
...
function mostrarMenuTareas() {
    ...
    do {
        ...
        if (!opcion || opcion.trim() === "") {//Controlo que no ingrese vacio
            console.error("Opción no válida");
            continue;
          }
